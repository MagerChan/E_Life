<?php
    ...
    if($id){
        $data = save_suggest($id,$evaluate,$orderid);
    }

    echo $_GET['jsoncallback']."(".json_encode($data).")";//解决ajax跨域问题
    

    function save_suggest($id,$evaluate,$orderid){
        ...
        try {
            ...
            $sql = "insert into `elife_evaluate` (`user_id`,`order_id`,`content`,`time`) values ($id,$orderid,'$evaluate','$timestamp')";
