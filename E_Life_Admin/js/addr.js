...
function get_addr_info(){
    $.ajax({
        url: 'http://elife.admin.com/addr.php',
        type: 'GET',
        dataType: 'json',
        timeout: 15000,
        error: function(){
...
function tmpl_create_table(tmpl_info){
    $("#addr_list").empty();
    var root = document.getElementById("addr_list");
    ...
    for (var i=0; i<tmpl_info.length; i++){
        ...
        <td>\
            <div style="width: 100%; height: 100%;text-align:center;">'+tmpl_info[i]["name"]+'</div>\
        </td>\
        <td>\
            <div style="width: 100%; height: 100%;text-align:center;">'+tmpl_info[i]["detail_addr"]+'</div>\
        </td>\
        <td>\
        ...
        <td>\
            <div style="width: 100%; height: 100%;text-align:center;">\
                <a class="tmpl_change_status" title="点击删除用户地址"><img class="icon_class" src="./img/delete.png"/></a>\
            </div>\
        </td>';
        $(root).append(tr);
...
function onDelete(obj){
	var addr_id=tmpl_info[$(obj).data('tmpl_info_pos')]['_id'];
	ELife_UI.Message.show({
		title: '',
		message: '确定要删除该地址吗?',
		ok: function() {
			$.ajax({
				url: 'http://elife.admin.com/addr_del.php?addr_id='+addr_id+'',
				type: 'GET',
				dataType: 'json',
				timeout: 15000,
				success: function(data) {
